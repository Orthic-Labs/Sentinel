...
function resolveDocs(input = {}, options = {}) {
  const baseRoot = path.resolve(options.projectRoot ?? process.cwd());
  const projectRoot = path.resolve(input.path ?? baseRoot);
  if (projectRoot !== baseRoot && !projectRoot.startsWith(`${baseRoot}${path.sep}`)) {
    throw new Error('docs path must stay inside project root');
  }
  const token = parseContinuation(input.continuationToken);
  const library = input.library ?? token?.library ?? inferLibrary(projectRoot);
  if (!library) throw new Error('docs requires library or a lockfile with one dependency');
  const packageInfo = resolvePackage(projectRoot, library, input.version ?? token?.version ?? 'auto');
  if (!packageInfo) throw new Error(`Could not resolve an exact installed version for ${library}; result is unverified`);
  const query = String(input.query ?? token?.query ?? '').trim();
  ...
  const offset = Number.isInteger(token?.offset) && token.offset > 0 ? token.offset : 0;
  const page = items.slice(offset, offset + pageSize);
  const nextOffset = offset + pageSize;
  const continuationToken = items.length > nextOffset ? Buffer.from(JSON.stringify({ library, version: packageInfo.version, query, offset: nextOffset })).toString('base64url') : undefined;
  if (options.store) {
...
function parseContinuation(value) {
  ...
  try {
    ...
    throw new Error('invalid docs continuationToken');
  }
}
