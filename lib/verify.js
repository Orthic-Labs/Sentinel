...
function evaluateGate(run, store, gate = 'signoff') {
  ...
  const trustedAuthorities = new Set(['operator', 'hook']);
  const passing = checks.filter((check) => check.status === 'passed' && trustedAuthorities.has(check.authority) && check.executor !== 'model_claim');
  ...
  const logOrDocEvidence = activeEvidence.filter((item) => ['official_doc', 'tool', 'verifier', 'user'].includes(item.trust_class) && ['logs', 'docs', 'observation', 'test'].includes(item.kind)).length;
  ...
  const unsupported = supportedMaterial.filter((claim) => !activeEvidence.some((item) => Array.isArray(item.claim_ids) && item.claim_ids.includes(claim.id)));
  if (unsupported.length) deficits.push({ code: 'supported_claim_without_evidence', ids: unsupported.map((claim) => claim.id) });
