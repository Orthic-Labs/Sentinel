...
const CORTEX_KINDS = new Set(['cortex_orientation', 'cortex', 'orientation']);

/**
 * Hook point for Cortex P1 orientation receipts.
 * When Cortex emits orientation evidence (kind cortex_orientation / cortex),
 * Forge accepts it as attested tool-class evidence under host/hook authority, or as
 * a provisional model_claim under model authority (still subject to gate minima).
 */
function acceptCortexOrientation(item = {}, authority = 'model') {
  const kind = item.kind ?? '';
  if (!CORTEX_KINDS.has(kind)) return { accepted: false, reason: 'not_cortex' };
  if (item.receipt || item.cortex_receipt || item.orientation_hash || item.excerpt) {
    return {
      accepted: authority === 'hook' || authority === 'operator' || Boolean(item.receipt || item.cortex_receipt),
      reason: 'cortex_orientation_receipt',
    };
  }
  return { accepted: false, reason: 'cortex_orientation_incomplete' };
}
...
function buildRubric(run, input = {}) {
  ...
  return {
    ...
    criteria: criteria.map((criterion, index) => {
      const object = typeof criterion === 'string' ? { criterion } : criterion;
      if (!object || typeof object !== 'object' || Array.isArray(object)) throw new Error(`rubric criterion ${index} must be a string or an object`);
      const text = [object.criterion, object.text].find((value) => typeof value === 'string' && value.trim())?.trim() ?? '';
      if (!text) throw new Error(`rubric criterion ${index} requires criterion text in "criterion" or "text"`);
...
function evidenceKindMatches(required, evidence) {
  ...
  return required.some((req) => {
    ...
    if (req === 'docs' && ['docs', 'official_doc', 'cortex_orientation', 'cortex'].includes(kind)) return true;
    if (req === 'logs' && ['logs', 'observation', 'test'].includes(kind)) return true;
    if (req === 'logs_or_docs' && ['logs', 'docs', 'observation', 'test', 'official_doc', 'cortex_orientation', 'cortex'].includes(kind)) return true;
    if (req === 'test' && kind === 'test') return true;
    if (req === 'cortex' && CORTEX_KINDS.has(kind)) return true;
    return false;
  });
}
...
function evaluateGate(run, store, gate = 'signoff') {
  ...
  const logOrDocEvidence = activeEvidence.filter((item) => (
    ...
    && ['logs', 'docs', 'observation', 'test', 'cortex_orientation', 'cortex'].includes(item.kind)
  )).length;
