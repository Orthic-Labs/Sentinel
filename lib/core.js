...
class SentinelCore {
  ...
  verify(input = {}, authority = this.authority) {
    ...
    return this.response(run, decision, gateResult?.deficits ?? [], [], { rubric_id: rubric.id, gate: gateResult, preflight, authority });
  }
  ...
  close(input = {}, authority = this.authority) {
    ...
    const entries = [{
      table: 'decisions',
      value: { run_id: run.id, decision: 'closed', summary: bounded(input.summary ?? run.summary, 1_024), ledger_hash: ledgerHash, authority, close_transaction_id: closeTransactionId },
    }];
    ...
    return this.response(closed, 'closed', [], [], { gate, ledger_hash: ledgerHash, authority });
  }
  ...
  state(run) {
    return {
      ...
      state_ref: `sentinel://run/${run.id}/state`, budget_used: run.budget,
    };
  }

  response(run, decision, nextActions = [], updates = [], extra = {}) {
    return {
      run_id: run.id, operation: extra.operation ?? 'sentinel', decision, summary: bounded(run.summary, 1_024),
      ...
      state_ref: `sentinel://run/${run.id}/state`, budget_used: run.budget, ...extra,
    };
  }
}
