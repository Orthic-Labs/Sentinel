...
class ReflectCore {
  constructor(options = {}) {
    ...
    this.store = options.store ?? createStore(options.storeRoot ?? process.env.REFLECT_STORE_ROOT ?? path.join(this.projectRoot, '.reflect'));
  ...
  checkpoint(input = {}, authority = this.authority) {
    ...
    for (const item of evidence.slice(0, 30)) this.store.append('evidence', normalizeEvidence(item, run.id, this.store));
    let retry = null;
    if (input.failure) {
  ...
  state(run) {
    return {
      ...
      state_ref: `reflect://run/${run.id}/state`, budget_used: run.budget,
    };
  }

  response(run, decision, nextActions = [], updates = [], extra = {}) {
    return {
      run_id: run.id, operation: extra.operation ?? 'reflect', decision, summary: bounded(run.summary, 1_024),
      ...
      state_ref: `reflect://run/${run.id}/state`, budget_used: run.budget, ...extra,
    };
  }
}
...
function normalizeEvidence(item, runId, store) {
  ...
  return {
    ...
    content_hash: item.content_hash ?? payload?.hash ?? null, retrieved_at: item.retrieved_at ?? new Date().toISOString(), observed_at: item.observed_at ?? new Date().toISOString(),
    trust_class: item.trust_class ?? item.trustClass ?? 'tool', freshness_policy: item.freshness_policy ?? 'event-only',
    invalidation_key: item.invalidation_key ?? item.invalidationKey ?? null, supports_or_refutes: item.supports_or_refutes ?? 'supports', excerpt,
