...
const { buildRubric, evaluateGate, runPreflightStub, acceptBlueprintOrientation } = require('./verify');
const { resolveDefaultStoreRoot } = require('./host');
...
class BeaconCore {
  ...
  verify(input = {}, authority = this.authority) {
    ...
    return this.response(run, decision, gateResult?.deficits ?? [], [], { rubric_id: rubric.id, gate: gateResult, preflight });
  }
  ...
  close(input = {}, authority = this.authority) {
    ...
    if (!gate.ok) {
      ...
      return this.response(run, 'blocked', gate.deficits.map((deficit) => `Signoff deficit: ${deficit.code}`), [], { gate });
    }
    ...
    const entries = [{
      table: 'decisions',
      value: { run_id: run.id, decision: 'closed', summary: bounded(input.summary ?? run.summary, 1_024), ledger_hash: ledgerHash, authority },
    }];
    for (const candidate of (input.memory_candidates ?? input.memoryCandidates ?? [])) {
      entries.push({ table: 'memory_candidates', value: { run_id: run.id, ...candidate, status: 'pending', authority: 'quarantine' } });
    }
    entries.push({ table: 'runs', value: { ...run, status: 'closed', close_ledger_hash: ledgerHash, closed_at: new Date().toISOString() } });
  ...
  state(run) {
    return {
      ...
      state_ref: `beacon://run/${run.id}/state`, budget_used: run.budget,
    };
  }

  response(run, decision, nextActions = [], updates = [], extra = {}) {
    return {
      run_id: run.id, operation: extra.operation ?? 'beacon', decision, summary: bounded(run.summary, 1_024),
      ...
      state_ref: `beacon://run/${run.id}/state`, budget_used: run.budget, ...extra,
    };
  }
}

/** @deprecated Use BeaconCore. Kept as a dual-alias during the Tether→Beacon rename. */
const ReflectCore = BeaconCore;

function normalizeCriteria(criteria) {
  return criteria.slice(0, 30).map((criterion, index) => {
    if (typeof criterion === 'string') return criterion.slice(0, 500);
    return {
      ...criterion,
      id: criterion.id ?? `criterion-${index}`,
      execution_contract: criterion.execution_contract ?? criterion.executionContract ?? null,
    };
  });
}
...
function normalizeEvidence(item, runId, store, projectRoot, authority = 'model') {
  ...
  const blueprint = acceptBlueprintOrientation(item, authority);
  ...
  let trustClass;
  if (authority === 'model') {
    if (blueprint.accepted) trustClass = 'tool';
    else if (attestation.attested) trustClass = 'tool';
    else trustClass = 'model_claim';
  } else if (hostReceipt) {
    trustClass = 'tool';
  } else {
    trustClass = item.trust_class ?? item.trustClass ?? (attestation.attested || blueprint.accepted ? 'tool' : 'model_claim');
  }

  const falsified = !attestation.attested && attestation.reason !== 'no_file_locator' && !blueprint.accepted;
  ...
  return {
    ...
    attested: hostReceipt || attestation.attested || blueprint.accepted,
    attestation_reason: hostReceipt ? 'host_receipt' : (blueprint.accepted ? blueprint.reason : attestation.reason),
    ...
    blueprint_orientation: blueprint.accepted,
  };
}
