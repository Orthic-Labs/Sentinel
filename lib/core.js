...
class ReflectCore {
  ...
  assess(input = {}, authority = this.authority) {
    ...
    const run = this.store.append('runs', {
      id: requestedRunId ?? crypto.randomUUID(), summary,
    ...
    for (const claim of claims) this.store.append('claims', normalizeClaim(claim, run.id, authority));
  ...
  resolveSession(input = {}) {
    ...
    if (existingRun && existingRun.status !== 'closed') return { run_id: existingRun.id, status: 'existing' };

    const requestedRunId = input.run_id ?? input.runId;
    const openRuns = this.store.list('runs').filter((run) => run.status !== 'closed');
    const run = requestedRunId ? this.store.get('runs', requestedRunId) : openRuns.length === 1 ? openRuns[0] : null;
    if (!run || run.status === 'closed') {
      return { run_id: null, status: openRuns.length > 1 ? 'ambiguous' : 'none' };
    }
    this.store.append('session_bindings', {
      id: sessionId, session_id: sessionId, run_id: run.id, project_root: this.projectRoot,
    });
...
function normalizeCheck(check, runId, store, authority = 'model') {
  ...
  const callerAuthority = authority === 'operator' ? 'operator' : authority === 'hook' ? 'hook' : 'model';
  return {
    id: check.id ?? crypto.randomUUID(), run_id: runId, kind: check.kind ?? 'script', specification: check.specification ?? check.command ?? '',
    command: check.command ?? null, executor: callerAuthority === 'model' ? 'model_claim' : check.executor ?? callerAuthority, authority: callerAuthority, env_fingerprint: check.env_fingerprint ?? null,
