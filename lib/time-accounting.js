...
// Forge side of workspace rule 6.6 (time-accounting loop): tasklist declares a planned
// minute budget, the host's tool-receipt hook already stamps real started_at/completed_at per
// tool call into Membrane's telemetry ledger, and close() reads the sum back through Forge's
// own scoped read path (query_observable_events_for_forge, tool-origin only) to score
...
// session), while a Forge run's task_id is caller-supplied and has no guaranteed relationship
...
// The crypt service is keyed by the api-token file living in the runtime config's
// sibling cache directory. The reader uses the same identity the host owns -- no credential
// is invented, the file is the only source of authority. Failure to read it is treated as
// the service being unavailable. The lookup walks up from `<workspace>/tools/lib/memory/`
// to `<workspace>/tools/.cache/memory/api-token` to match the crypt-service's own layout.
function resolveApiToken(configPath = RUNTIME_CONFIG_PATH) {
  const tokenPath = path.join(path.dirname(configPath), '..', '..', '.cache', 'memory', 'api-token');
  try {
...
function postJson(host, port, urlPath, body, timeoutMs, bearerToken) {
  return new Promise((resolve, reject) => {
    ...
    const request = http.request(
      {
        host, port, path: urlPath, method: 'POST', timeout: timeoutMs, headers,
      },
      (response) => {
        const chunks = [];
...
/**
 * Sum tool-origin duration_ms for a session via Forge's scoped Membrane read path
 * (:query-forge-time -> query_observable_events_for_forge, tool-origin only). Returns null
...
async function actualActiveMinutes({ sessionId, sinceIso, configPath, limit = 1_000 } = {}) {
  ...
  const bearerToken = resolveApiToken(configPath ?? RUNTIME_CONFIG_PATH);
  try {
