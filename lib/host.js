...
function resolveHostDataDir() {
  const override = envFirst('FORGE_HOST_DATA');
  if (override) return path.resolve(override);
  return platformDataRoot('forge');
}
...
function isTrustedWorkspaceHostExecutable(executable, options = {}) {
  ...
  const allowedSuffixes = [
    '/ForgeHost.app/Contents/MacOS/ForgeHost',
    '/ForgeHost.app/Contents/Resources/rhook/rhook',
  ];
...
function isTrustedWindowsWorkspaceHostExecutable(executable, options = {}) {
  ...
  const expected = [
    path.win32.resolve(localAppData, 'ForgeHost', 'rhook.exe'),
    path.win32.resolve(localAppData, 'ForgeHost', 'rhook', 'rhook.exe'),
...
function resolveDefaultStoreRoot(projectRoot) {
  ...
  // cannot write. FORGE_HOST_DATA is agent-controllable (any Bash call can export it), so if it
  ...
  if (resolvedHostDataDir === resolvedProject || resolvedHostDataDir.startsWith(resolvedProject + path.sep)) {
    throw new Error(
      `forge: refusing agent-writable authority ledger — FORGE_HOST_DATA (${hostDataDir}) resolves inside the project workspace (${resolvedProject}); `
      + 'the ledger must live outside any directory the agent can write to. Unset FORGE_HOST_DATA or point it at a host-owned directory.',
    );
  }
...
function doctor(projectRoot = process.cwd()) {
  ...
  const pluginRoot = envFirst('CODEX_PLUGIN_ROOT', 'CLAUDE_PLUGIN_ROOT', 'FORGE_PLUGIN_ROOT');
  const findings = [];
  let storeRoot = null;
  ...
  if (storeRoot === null) {
    ...
    findings.push({ level: 'info', code: 'store_uninitialized', message: 'store is created only by a normal Forge operation' });
  } else {
    try {
