...
function main() {
  ...
  let result = { action: 'noop', reason: 'forge hook unavailable' };
  try { result = evaluate(event, { root }); } catch { /* Fail open when Forge cannot evaluate its own result. */ }
  if (result.action !== 'block') return;
  const eventName = event.hook_event_name ?? event.event ?? event.type ?? '';
  const reason = String(result.reason ?? 'forge gate unmet').slice(0, 500);
