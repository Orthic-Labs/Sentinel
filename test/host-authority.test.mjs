...
test("an agent-controlled SENTINEL_HOST_DATA pointing inside the project workspace is rejected loudly, not silently downgraded", () => {
  // SENTINEL_HOST_DATA is read from the environment, and the environment is exactly what a
  // shell the agent runs can set (`export SENTINEL_HOST_DATA=...`). If Sentinel silently accepted
  ...
  try {
    ...
    process.env.SENTINEL_HOST_DATA = join(project, "subdir");
    assert.throws(() => resolveDefaultStoreRoot(project), /agent-writable/);
    ...
    process.env.SENTINEL_HOST_DATA = join(root, "legit-host");
...
test("doctor reports an agent-writable SENTINEL_HOST_DATA as a loud error finding instead of crashing", () => {
  const root = mkdtempSync(join(tmpdir(), "sentinel-doctor-forged-"));
  const project = join(root, "project");
