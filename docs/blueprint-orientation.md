...
Beacon accepts Blueprint orientation receipts when present; it does not require Blueprint to ship first.
...
When Blueprint P1 emits a stable receipt schema, map it here without changing Beacon ops (`assess` / `checkpoint` / `verify` / `close`).
