...
`tools/.cache/metrics/forge/` with the same bounded format and opt-out
(`FORGE_TELEMETRY_ENABLED=0`), so read both paths when computing the Phase 0 baseline — the older
directory is the pre-rename history and is not migrated. v1.1 regression coverage remains in
`test.mjs`; its launch sets `FORGE_LEGACY_TOOLS=1` so the compatibility behavior is exercised
explicitly.
