...
opt-out (`TETHER_TELEMETRY_ENABLED=0`). v1.1 regression coverage remains in `test.mjs`; its launch
sets `TETHER_LEGACY_TOOLS=1` so the compatibility behavior is exercised explicitly.
