...
### P0 (prior)
- Trust boundary: host token file replaces `TETHER_TRUSTED_CALLER`; default store moves to host data directory with legacy `.tether/` fallback (`docs/store-migration.md`).
...
- Rename surfaces: `BeaconCore` (`ReflectCore` alias), `beacon`/`tether` bins + MCP tools, `BEACON_*`/`TETHER_*` env dual-alias, Codex hooks prefer `CODEX_PLUGIN_ROOT`, `init`/`doctor`, e2e lifecycle coverage.
...
- Replaced the v1 MCP surface with `tether` and `docs`; legacy aliases are migration errors.
